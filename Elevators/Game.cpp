...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    while (!gameFile.eof()) {
        ...
        Person p(fileLine);
        int personTurn = p.getTurn();

        // User plays the game until the turn is reached
        while (building.getTime() < personTurn) {
            building.prettyPrintBuilding(cout);
            satisfactionIndex.printSatisfaction(cout, false);
            checkForGameEnd();

            Move nextMove = getMove(); // AI mode check is within this function
            update(nextMove);
        }

        // Spawn the person when the turn is reached
        building.spawnPerson(p);
    }

    // After all people have spawned, the user continues to play the game
    while (true) {
        building.prettyPrintBuilding(cout);
        satisfactionIndex.printSatisfaction(cout, false);
        checkForGameEnd();
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    int l = pickupList.size();
    
    // Condition 1: Unique indices
    for (int i = 0; i < l; i++) {
        for (int j = i + 1; j < l; j++) {
    ...
    // Condition 2: Digits only
    for (int i = 0; i < l; i++) {
        if (!isdigit(pickupList[i])) {
            return false;
        }
    }
    
    // Condition 3: Capacity check
    if (l > ELEVATOR_CAPACITY) {
        return false;
    }
    
    // Condition 4: Index within range
    int peopleOnFloor = building.getFloorByFloorNum(pickupFloorNum).getNumPeople();
    for (int i = 0; i < l; i++) {
        int personIndex = pickupList[i] - '0'; // Convert char to int
        if (personIndex < 0 || personIndex >= peopleOnFloor) {
            return false;
        }
    }
    
    // Condition 5: Direction consistency
    bool hasUp = false, hasDown = false;
    for (int i = 0; i < l; i++) {
        int personIndex = pickupList[i] - '0'; // Convert char to int
        const Person& person = building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(personIndex);
        if (person.getTargetFloor() > pickupFloorNum) {
            hasUp = true;
        } else if (person.getTargetFloor() < pickupFloorNum) {
            hasDown = true;
        }
    }
    if (hasUp && hasDown) {
